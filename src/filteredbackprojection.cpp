...
void FBP::destroy (void){
    if (fft_proj      != NULL)  free(fft_proj);
    if (sines         != NULL)  free(sines);
    if (cosines       != NULL)  free(cosines);
    if (filter_lut    != NULL)  free(filter_lut);
    if (filtered_proj != NULL)  free(filtered_proj);
}
