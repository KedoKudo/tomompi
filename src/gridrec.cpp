...
void GridRec::setSinoAndReconBuffers (  int  number, 
                                      float *sinogram_address, 
                                      float *reconstruction_address){
    int     loop;

    if (G1 == NULL)  G1 = (float **) malloc((size_t) (theta_list_size * sizeof(float *)));
    if (G2 == NULL)  G2 = (float **) malloc((size_t) (theta_list_size * sizeof(float *)));
    if (S1 == NULL)  S1 = (float **) malloc((size_t) (imgsiz * sizeof(float *)));
    if (S2 == NULL)  S2 = (float **) malloc((size_t) (imgsiz * sizeof(float *)));
...
void GridRec::init (void){
    ...
    H = (complex *) malloc ((M+1)*(M+1)*sizeof(complex));
...
void GridRec::destroy (void){
    ...
    if (COSE     != NULL)  free (COSE);
    if (cproj    != NULL)  free (cproj);
    if (filphase != NULL)  free (filphase);

    if (wtbl != NULL)  free (wtbl);
    if (winv != NULL)  free (winv);
    ...
    //Freeing this memory causes problems for some reason...
    //if (H != NULL)  free (H);
...
void GridRec::filphase_su (   long  pd, 
                             float  center, 
                           complex *A){
/******************************************************************/
